...
static void out_stopped_handler(Animation *animation, bool finished, void *context) {
  ...
  GRect start = GRect(frame.origin.x + (2 * ANIMATION_WINDOW_DISTANCE), frame.origin.y, frame.size.w, frame.size.h);
  GRect finish = GRect(frame.origin.x + ANIMATION_WINDOW_DISTANCE, frame.origin.y, frame.size.w, frame.size.h);
  ...
  animation_set_duration(in_anim, ANIMATION_WINDOW_DURATION);
...
static void shake_animation() {
  ...
  GRect finish = GRect(start.origin.x - ANIMATION_WINDOW_DISTANCE, start.origin.y, start.size.w, start.size.h);
  ...
  animation_set_duration(out_anim, ANIMATION_WINDOW_DURATION);
...
static void animate() {
  shake_animation();
  s_timer = app_timer_register(ANIMATION_WINDOW_INTERVAL, animate_callback, NULL);
}
